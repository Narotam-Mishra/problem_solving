
// Problem link - https://leetcode.com/problems/calculate-money-in-leetcode-bank/?envType=daily-question&envId=2025-10-25

#include<bits/stdc++.h>
using namespace std;

int totalMoney1(int n) {
    // store total amount of money in `total`
    int total = 0;

    // step 1 - iterate on each day
    for (int i = 0; i < n; i++) {
        // step 2 - find number of week
        int w = i / 7;

        // step 3 - find number of day
        int d = i % 7;

        // step 4 - calculate total
        total += (w + d + 1);
    }

    // step 5 - return total amount of money
    return total;
}

int totalMoney(int n) {
    // step 1 - calculate number of complete and remaining weeks
    int completeWeeks = n / 7;
    int remainingDays = n % 7;
    int total = 0;

    // step 2 - calculate money from complete weeks
    if(completeWeeks > 0){
        // sum of arithmetic sequence: 7k + 21 for k from 1 to completeWeeks
        int weekSum = 7 * completeWeeks * (completeWeeks + 1) / 2 + 21 * completeWeeks;
        total += weekSum;
    }

    // step 3 - calculate money from remaining days in the incomplete week
    if(remainingDays > 0){
        // Monday of the incomplete week
        int startVal = completeWeeks + 1;

        // sum = number_of_terms * (first_term + last_term) / 2
        int remainingSum = remainingDays * (startVal + startVal + remainingDays - 1) / 2;

        total += remainingSum;
    }

    // step 4 - return the total amount of money at nth day
    return total;
}

int main(){
    int n = 4;

    // int n = 10;

    // int n = 20;
    cout<<totalMoney(n);
    return 0;
}