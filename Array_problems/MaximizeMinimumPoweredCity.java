
// Problem link - https://leetcode.com/problems/maximize-the-minimum-powered-city/?envType=daily-question&envId=2025-11-07

package Array_problems;

public class MaximizeMinimumPoweredCity {
    // utility function to check if it's possible to achieve minimum power of 'minPower' for all cities
    private static boolean  canAchieveMinPower(long[] diff, long minPower, int r, int k, int n){
        long[] tempDiff = diff.clone();

        // iterate through each city to check if minPower is achievable
        long currPower = 0;
        for(int i=0; i<n; i++){
            // update current city's power using DAT
            currPower += tempDiff[i];

            // if current city has less power than required minimum
            if(currPower < minPower){
                // calculate how many power stations we need to add
                long psNeed = minPower - currPower;

                // if we don't have enough power stations left, this target is
                // not achievable
                if(psNeed > k){
                    return false;
                }

                //  use the required power stations
                k -= psNeed;

                // using DAT
                currPower += psNeed;
                if(i + 2*r + 1 < n){
                    tempDiff[i+2*r+1] -= psNeed;
                }
            }
        }

        // return true, as successfully achieved minPower for all cities
        return true;
    }
    
    public static  long maxPower(int[] stations, int r, int k) {
        int n = stations.length;

        // step 1 - build initial difference array to calculate power for each
        // city using Difference Array Technique (DAT) diff[i] = the change in
        // power contribution at city i
        long[] diff = new long[n];
        for(int i=0; i<n; i++){
            // range should not be out of index
            diff[Math.max(0, i-r)] += stations[i];

            if(i + r + 1 < n){
                diff[i+r+1] -= stations[i];
            }
        }

        // step 2 - using binary search on answer to find maximum possible
        // minimum power lower bound: minimum power any city currently has
        long s = Long.MAX_VALUE;
        for(int st : stations){
            s = Math.min(s, st);
        }

        // upper bound: if we place all k stations optimally, maximum possible power
        long e = k;
        for(int st : stations){
            e += st;
        }

        long res = 0;
        while(s <= e){
            long mid = s + (e - s)/2;

            // check if we can make every city have at least 'mid' power
            if(canAchieveMinPower(diff, mid, r, k, n)){
                // if achievable, try for a higher minimum power
                res = mid;
                s = mid + 1;
            }else{
                //  if not achievable, try for a lower minimum power
                e = mid - 1;
            }
        }

        // step 3 - return final result, res
        return res;
    }

    public static void main(String[] args) {
        // int[] stations = {1,2,4,5,0};
        // int r = 1, k = 2;

        int[] stations = {4,4,4,4};
        int r = 0, k = 3;
        System.out.println(maxPower(stations, r, k));
    }
}
